...
def filter(sourList,filterList):
    dest=[]
...
def getContextByNode(htmlSoup,titleKeyWordList,filterList):
    ...
    sentenceList=filter(sentenceList,excludeWords)
    sentenceList=filter(sentenceList,filterList)
...
def getHTMLContext(html,filterList):
    ...
    titleKeyWordList=filter(titleKeyWordList,excludeWords)
    titleKeyWordList=filter(titleKeyWordList,filterList)

    if len(titleKeyWordList)==0:
        return ""

    markWindowsList=getMarkWindowsList(htmlSoup.html.body)
    markWindowsList.sort(lambda x,y: cmp(x.posi, y.posi))

    context=""
    for item in markWindowsList:
        innerText=item.innerText
        markWindowKeyWordList=jieba.cut(innerText)
        markWindowKeyWordList=filter(markWindowKeyWordList,excludeWords)
        markWindowKeyWordList=filter(markWindowKeyWordList,filterList)
...
def encodeSpecialTag(html):
    ...
    zhushi=re.compile(r"<!--[.\s\S]*?-->")
    html=zhushi.sub('', html)
    ...
    while len(nodeQueue)>0:
        ...
        for childNode  in currNode:
            if childNode.string!=None:
                ...
                if tmp.split(' ')<=2 and len(innerText)-tmpInt>tmpInt:continue
